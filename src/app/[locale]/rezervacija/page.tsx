...
export const metadata: Metadata = {
  title: 'Rezervacija',
  description: 'Rezervišite vaš termin u Majstor i Margarita.',
};
...
export default async function RezervacijaPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  try {
    ...
    const { data } = await supabase.from('packages').select('*').eq('is_active', true).neq('slug', 'corporate').order('sort_order');
  ...
  return (
    <>
      ...
      <div className="pt-16"><BookingWizard packages={packages} /></div>
    </>
  );
}
