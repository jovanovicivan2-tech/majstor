          ...
          <div className="grid grid-cols-2 gap-px overflow-hidden" style={{ background: 'rgba(138,126,114,0.12)' }}>
            {FEATURES.map((f) => (
              <div key={f.label} className="p-7 text-center" style={{ background: '#F7F2EA' }}>
                ...
                <p className="text-[11px] font-semibold tracking-[0.12em] uppercase mb-1" style={{ color: '#8A7E72' }}>{f.label}</p>
                <p className="text-[13px]" style={{ color: '#1C1C1E' }}>{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
