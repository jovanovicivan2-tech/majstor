...
export default function ChatWidget() {
  ...
  return (
    <>
      ...
      {open && (
        <div style={{ position: 'fixed', bottom: 152, right: 16, zIndex: 40, width: 'calc(100vw - 32px)', maxWidth: 360, background: 'white', border: '1px solid #EDE8E0', borderRadius: 16, overflow: 'hidden', boxShadow: '0 24px 70px rgba(28,28,30,0.22)', display: 'flex', flexDirection: 'column', maxHeight: 480 }}>
          <div style={{ background: 'linear-gradient(135deg, #5C1A2E, #43101F)', padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 16, background: '#F7F2EA', display: 'flex', flexDirection: 'column', gap: 12 }}>
            {msgs.map((m, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                <div style={{ maxWidth: '82%', padding: '8px 12px', borderRadius: 10, fontSize: 14, lineHeight: 1.5, background: m.role === 'user' ? '#5C1A2E' : 'white', color: m.role === 'user' ? 'white' : '#1C1C1E', border: m.role === 'user' ? 'none' : '1px solid #EDE8E0' }}>{m.content}</div>
              </div>
            ))}
            {loading && <div style={{ display: 'flex', justifyContent: 'flex-start' }}><div style={{ padding: '8px 12px', background: 'white', border: '1px solid #EDE8E0', borderRadius: 10, display: 'flex', gap: 4 }}>{'···'}</div></div>}
            <div ref={ref} />
          </div>
          {msgs.length <= 2 && (
            <div style={{ padding: '8px 12px', display: 'flex', gap: 8, overflowX: 'auto', background: 'white', borderTop: '1px solid #EDE8E0', flexShrink: 0 }}>
              {QR.map(q => <button key={q} onClick={() => send(q)} style={{ flexShrink: 0, fontSize: 12, padding: '0 12px', height: 32, border: '1px solid rgba(92,26,46,0.3)', color: '#5C1A2E', background: 'transparent', borderRadius: 10, cursor: 'pointer', whiteSpace: 'nowrap' }}>{q}</button>)}
            </div>
          )}
          <div style={{ padding: 12, borderTop: '1px solid #EDE8E0', display: 'flex', gap: 8, background: 'white', flexShrink: 0 }}>
            <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()} placeholder="Unesite pitanje..." style={{ flex: 1, height: 40, padding: '0 12px', border: '1px solid #D5CDC3', borderRadius: 10, fontSize: 14, outline: 'none' }} />
            <button onClick={() => send()} disabled={!input.trim() || loading} style={{ width: 40, height: 40, background: '#5C1A2E', color: 'white', border: 'none', borderRadius: 10, cursor: 'pointer', fontSize: 18, opacity: (!input.trim() || loading) ? 0.4 : 1, flexShrink: 0 }}>→</button>
          </div>
        </div>
      )}
    </>
